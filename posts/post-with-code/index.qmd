...
```{r}
Gene <- unique(Proteomic.Data$Gene[abs(Proteomic.Data$logFC) >= 1.5 & Proteomic.Data$P.Value <= 0.055])

print( paste(length(Gene), "out of", length(unique(Proteomic.Data$Gene)), "proteins were significantly affected in at least one mutant condition; Here we consider any protein whose steady state level changed by at least 1.5 (|Log2FC|)" ))
```
...
text(x = 1.45, y = 22, labels = "Decreased steady state levels \n p value < 0. 55", pos = 4, offset = .5)
text(x = -1.45, y = 22, labels = "Increased steady state levels \n p value < 0. 55", pos = 2, offset = .5)

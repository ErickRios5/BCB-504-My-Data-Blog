...
Here in the U.S. any citizen has the right to own firearms. However, the proliferation of firearms has many concerned on public safety and crimes committed with firearms. From your average Joe to politicians at the federal level, opinions differ on the threats posed and consequences of wide-spread availability of firearms, ability to conceal or openly carry a firearm.
...
I hereby consider a **change in the number of casualties**, as a measure of increasing/decreasing gun violence.

## Data

Figures made and presented in this post will be based mainly on a U.S. Gun Violence Data set containing gun violence incidents, retrieved from [Kaggle.com](https://www.kaggle.com/datasets/jasonmobley/united-states-gun-violence-data-20142022 "Gun Violence data 2014-2022") on 3/21/2023. However, the data set was originally collected from [gunviolencearchive.org](https://www.gunviolencearchive.org/).
...
Figure 1: Line plot displaying number of incidents as a function of time, from years 2014 to 2022.

From years 2014 to 2020 there was a substantial increase in the number of incidents in the United States. However, from years 2020 to 2022, there was a substantial decrease in gun violence incidents, so much so that the total number of incidents are below 2014 levels.

Lets take a closer look at the data to see if the overall patterns persist when separated by categorical values such as deaths, injuries, both, or neither.
...
Figure 2: Line scatter plot of total incidents, separated by casualty type, as a function of time from years 2014 to 2022.

The data first suggested the U.S. was experiencing less gun violence nationwide (Figure 1). When focusing on the total number of violent gun incidents decreased between the years 2020 and 2022, we can see that the incidents where no deaths or injuries occurred dipped drastically. However, the amount of deaths and/or injuries per year stayed fairly constant or only slightly increased!

This seems to suggest that gun violence each year is becoming deadlier. Could we possibly reflect that by introducing a new channel like size of each point based on the average number of deaths and/or injuries per incident for each year?
...
I believe we have reached a limit with the amount of data we can display on a scatter plot idiom. We could continue to add additional channels to convey more information, but the last addition of point size to convey the average number of deaths/injuries per incident does not work very well. This due to the average number of deaths or injuries per year being fairly constant and outliers have negligible impact on the point size when there are 40000+ incidents with only death or injury each year.
...
When only taking into account the number of isolated incidents, there has been a sharp decrease in gun violent incidents across the entire U.S.
...
2.  On a by state basis, the data suggests a few states, for some unknown reason are experiencing a surge in gun violence, while other states had less violence and most hover in-between.

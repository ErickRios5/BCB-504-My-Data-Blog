...
# example data with county names in a column called "county"
county_data <- data.frame(
  county = c("Los Angeles County", "Cook County", "Harris County")
)

# use tidycensus to get county FIPS codes
county_data$county_fips <- countycode(state = "*", county = county_data$county, 
                                      vintage = "acs5", year = 2019)

```

```{r}
counties_sf <- get_urbn_map(map = "counties", sf = TRUE)
counties_sf$county_name <- gsub("\\s\\w+$", "", counties_sf$county_name)
counties_sf$county_name <-tolower(counties_sf$county_name)

gundata$City_Or_County <- tolower(gundata$City_Or_County)
colnames(gundata)[colnames(gundata) == "City_Or_County"] <- "county_name"
...
gundata <- left_join(gundata, counties_sf, by = "county_name")

household_data <- left_join(county_groups, counties_sf, by = "county_fips")

household_data %>%
  ggplot() +
  geom_sf(data = household_data, aes(geometry = geometry, fill = cat_var)) +
